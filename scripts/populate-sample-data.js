  ...
  try {
    ...
    for (const partner of partners) {
      try {
        ...
        console.log(`  ⚠️ Partner might exist: ${partner.business_name}`);
      }
    }
    ...
    // 3. Get partner IDs
    console.log('📝 Fetching partner IDs...');
    ...
    for (const listing of listings) {
      ...
      if (!partnerId || !categoryId) {
        console.log(`  ⚠️ Skipping ${listing.title} - missing partner or category`);
        skipCount++;
        continue;
      }
      ...
      try {
        ...
        console.log(`  ⚠️ Could not create ${listing.title}:`, err.message);
        skipCount++;
      }
    }
