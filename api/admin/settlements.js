 ...
 *
 * 렌트카, 숙박 예약의 결제 내역을 집계하여 정산 데이터 제공
 */
...
async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      console.log(`✅ 렌트카 정산 조회: ${rentcarResult.rows?.length || 0}개`);
    } catch (e) {
      console.error('렌트카 정산 쿼리 오류:', e.message);
      errors.push('렌트카: ' + e.message);
    }

    // 2. 숙박 정산 조회 (lodging_bookings 테이블)
    try {
      let lodgingQuery = `
        SELECT
      ...
      console.log(`✅ 숙박 정산 조회: ${lodgingResult.rows?.length || 0}개`);
    } catch (e) {
