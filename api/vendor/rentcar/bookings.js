...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const bookings = (result.rows || []).map(row => ({
      ...
      pickup_datetime: row.pickup_date ? `${new Date(row.pickup_date).toISOString().split('T')[0]} ${row.pickup_time || ''}` : null,
      dropoff_date: row.dropoff_date ? new Date(row.dropoff_date).toISOString().split('T')[0] : null,
      dropoff_time: row.dropoff_time,
      dropoff_datetime: row.dropoff_date ? `${new Date(row.dropoff_date).toISOString().split('T')[0]} ${row.dropoff_time || ''}` : null,
      ...
      driver_license_no: row.driver_license_number,
