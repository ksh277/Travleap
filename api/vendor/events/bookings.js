...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.log('📋 [Event Vendor] 예약 조회:', { partner_id, role: decoded.role });
    ...
    const bookings = (result.rows || []).map(booking => {
      ...
      if (booking.customer_info) {
        try {
          const parsed = typeof booking.customer_info === 'string'
            ? JSON.parse(booking.customer_info)
            : booking.customer_info;
          customerInfo = parsed;

          // ticket_type과 quantity 추출
          if (parsed.ticket_type) ticketInfo.ticket_type = parsed.ticket_type;
          if (parsed.quantity) ticketInfo.quantity = parsed.quantity;
        } catch (e) {
    ...
    const statsResult = await connection.execute(
      `SELECT
        ...
        SUM(b.num_adults + b.num_children) as total_tickets
       FROM bookings b
       INNER JOIN listings l ON b.listing_id = l.id
    ...
    console.log('✅ [Event Vendor] 조회 완료:', {
    ...
    console.error('❌ [Event Vendor Bookings API] 오류:', error);
