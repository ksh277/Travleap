...
/**
 * 체험 예약 상태 변경 API
 * POST /api/vendor/experience/update-status
 *
 * Body:
 * - booking_id: 예약 ID
  ...
  try {
    ...
    if (vendor_id) {
      bookingCheck = await connection.execute(
        `SELECT eb.id, eb.status, e.vendor_id
         FROM experience_bookings eb
         INNER JOIN experiences e ON eb.experience_id = e.id
         WHERE eb.id = ? AND e.vendor_id = ?`,
        [booking_id, vendor_id]
      );
    } else {
      bookingCheck = await connection.execute(
        'SELECT id, status FROM experience_bookings WHERE id = ?',
        [booking_id]
      );
    }
    ...
    const updateResult = await connection.execute(
      'UPDATE experience_bookings SET status = ?, updated_at = NOW() WHERE id = ?',
      [status, booking_id]
    );
    ...
    console.error('❌ [Experience Update Status API] 오류:', error);
    return res.status(500).json({
      success: false,
      error: error.message
    });
