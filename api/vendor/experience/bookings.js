...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.log('📋 [Experience Vendor] 예약 조회:', { partner_id, role: decoded.role });
    ...
    const bookings = (result.rows || []).map(booking => {
      ...
      if (booking.customer_info) {
        try {
          ...
          // 체험 시간 추출
          experienceTime = customerInfo.experience_time || '';
        } catch (e) {
    ...
    const statsResult = await connection.execute(
      `SELECT
        ...
        SUM(COALESCE(b.num_adults, 0) + COALESCE(b.num_children, 0) + COALESCE(b.num_infants, 0)) as total_participants
       FROM bookings b
       INNER JOIN listings l ON b.listing_id = l.id
    ...
    console.log('✅ [Experience Vendor] 조회 완료:', {
    ...
    console.error('❌ [Experience Vendor Bookings API] 오류:', error);
