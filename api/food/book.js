...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const allowedDifference = Math.max(calculatedSubtotal * 0.3, 10000); // 30% 또는 최소 10,000원

    if (priceDifference > allowedDifference) {
      console.error(`❌ [Food Book] 가격 검증 실패!
        - 계산된 subtotal: ${calculatedSubtotal}원
        - 클라이언트 total: ${finalTotalAmount}원
        - 차이: ${priceDifference}원
        - 허용 범위: ${allowedDifference}원`);
    ...
    console.log(`✅ [Food Book] 가격 검증 완료: subtotal=${calculatedSubtotal}원, total=${finalTotalAmount}원, 차이=${priceDifference}원`);
    ...
    // 예약 번호 생성 (FOOD-YYYYMMDD-XXXX 형식)
    const today_str = new Date().toISOString().slice(0, 10).replace(/-/g, '');
    const randomNum = Math.floor(1000 + Math.random() * 9000);
    const bookingNumber = `FOOD-${today_str}-${randomNum}`;

    // ✅ 인원 정보 처리 (adults/children/infants 또는 party_size)
    const adultsCount = adults !== undefined ? parseInt(adults) : partySizeCount;
    const childrenCount = children !== undefined ? parseInt(children) : 0;
    const infantsCount = infants !== undefined ? parseInt(infants) : 0;
