...
export function AttractionsVendorDashboard() {
  ...
  const handleEditAttraction = (attraction: Attraction) => {
    setAttractionForm({
      ...
      short_description: attraction.description?.substring(0, 100) || '',
      description_md: attraction.description || '',
      price_from: attraction.admission_fee_adult || attraction.price_from || 0,
      location: attraction.location || '',
      address: attraction.address || '',
      max_capacity: attraction.max_capacity || 100,
  ...
  const handleSaveAttraction = async () => {
    if (!attractionForm.title) {
      toast.error('관광지명을 입력해주세요.');
      return;
    }
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const url = isEditingAttraction
        ? `/api/vendor/listings?id=${editingAttractionId}`
        : '/api/vendor/listings';
      const method = isEditingAttraction ? 'PUT' : 'POST';
      ...
      if (result.success) {
        toast.success(isEditingAttraction ? '관광지가 수정되었습니다.' : '관광지가 추가되었습니다.');
        resetAttractionForm();
        loadDashboardData();
      } else {
  ...
  const handleDeleteAttraction = async (attractionId: number) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`/api/vendor/listings?id=${attractionId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      const result = await response.json();
      if (result.success) {
        toast.success('관광지가 삭제되었습니다.');
        loadDashboardData();
      } else {
        toast.error(result.message || '삭제에 실패했습니다.');
      }
    } catch (error) {
      console.error('관광지 삭제 오류:', error);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <Tabs defaultValue="orders" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="attractions">관광지 관리</TabsTrigger>
          ...
          <TabsContent value="attractions" className="space-y-4">
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle>내 관광지 관리</CardTitle>
                    <CardDescription>관광지를 추가하고 관리하세요</CardDescription>
                  </div>
                  {!isAddingAttraction && !isEditingAttraction && (
                    <Button onClick={handleAddAttraction} className="gap-2">
                  ...
                  <div className="mb-6 p-6 border rounded-lg bg-gray-50">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold">
                        {isEditingAttraction ? '관광지 수정' : '새 관광지 추가'}
                      </h3>
                      <Button
                        variant="ghost"
                        size="sm"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="title">관광지명 *</Label>
                        <Input
                          id="title"
                          value={attractionForm.title}
                          onChange={(e) => setAttractionForm(prev => ({ ...prev, title: e.target.value }))}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          value={attractionForm.short_description}
                          onChange={(e) => setAttractionForm(prev => ({ ...prev, short_description: e.target.value }))}
                          placeholder="관광지 한줄 소개"
                        />
                      </div>

                      <div className="space-y-2">
                        <Label htmlFor="price_from">입장료 (원)</Label>
                        <Input
                          id="price_from"
                          type="number"
                          value={attractionForm.price_from}
                          onChange={(e) => setAttractionForm(prev => ({ ...prev, price_from: parseInt(e.target.value) || 0 }))}
                          placeholder="성인 기준 입장료"
                        />
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          id="max_capacity"
                          type="number"
                          value={attractionForm.max_capacity}
                          onChange={(e) => setAttractionForm(prev => ({ ...prev, max_capacity: parseInt(e.target.value) || 0 }))}
                          placeholder="시간대당 최대 입장 인원"
                        />
                      </div>
                      ...
                      <div className="space-y-2 flex items-center gap-2 pt-6">
                        <Switch
                          id="is_active"
                          checked={attractionForm.is_active}
                          onCheckedChange={(checked) => setAttractionForm(prev => ({ ...prev, is_active: checked }))}
                        />
                        <Label htmlFor="is_active">운영중</Label>
                      </div>
                      ...
                      <div className="space-y-2 md:col-span-2">
                        ...
                        <Textarea
                          id="description_md"
                          value={attractionForm.description_md}
                          onChange={(e) => setAttractionForm(prev => ({ ...prev, description_md: e.target.value }))}
                          placeholder="관광지 상세 설명을 입력하세요"
                          rows={4}
                        />
                      </div>
                      ...
                      <div className="space-y-2 md:col-span-2">
                        <Label>관광지 이미지</Label>
                        <ImageUploader
                          images={attractionForm.images}
                          onImagesChange={(images) => setAttractionForm(prev => ({ ...prev, images }))}
                          maxImages={5}
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-2 mt-6">
                      ...
                      <Button
                        ...
                        disabled={isSaving || !attractionForm.title}
                      >
                        {isSaving ? (
                          <>
                          ...
                          isEditingAttraction ? '수정 완료' : '추가 완료'
                        )}
                      </Button>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {attractions.length === 0 ? (
                    <div className="col-span-full text-center py-12 text-gray-500">
                      <p>등록된 관광지가 없습니다.</p>
                      <p className="text-sm mt-2">위의 "관광지 추가" 버튼을 눌러 관광지를 등록하세요.</p>
                    </div>
                  ) : (
                        ...
                        <CardContent>
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium text-right text-xs">{attraction.address || '-'}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">성인 요금</span>
                              <span className="font-semibold text-blue-600">
                                {(attraction.admission_fee_adult || attraction.price_from || 0).toLocaleString()}원
                              </span>
                            </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-600">관광지명:</span>
                      <span className="font-medium">{selectedOrder.attraction_name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">방문 예정일:</span>
                      <span className="font-medium">{new Date(selectedOrder.visit_date).toLocaleDateString('ko-KR')}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="space-y-2 text-sm">
                      {selectedOrder.tickets.map((ticket: any, index: number) => (
                        <div key={index} className="flex justify-between">
                          <span className="text-gray-600">{ticket.type_name || '티켓'}:</span>
