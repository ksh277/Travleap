    ...
    db.run('DELETE FROM reviews', function(err) {
      ...
      reviewsData.forEach((review, index) => {
        stmt.run([
          ...
          if (err) {
            console.error(`리뷰 ${index + 1} 삽입 실패:`, err);
          } else {
            insertedCount++;
            console.log(`리뷰 ${index + 1} 삽입 완료`);
          }
          ...
          if (index === reviewsData.length - 1) {
            ...
            // 리스팅 평점 업데이트
            console.log('리스팅 평점 업데이트 중...');
            const updateStmt = db.prepare('UPDATE listings SET rating_avg = 5.0, rating_count = 1 WHERE id = ?');
            const listingIds = [1, 2, 3, 4, 5, 11, 15, 16];

            let updatedCount = 0;
            listingIds.forEach((id, idx) => {
              updateStmt.run(id, function(err) {
                if (err) {
                  console.error(`리스팅 ${id} 평점 업데이트 실패:`, err);
                } else {
                  updatedCount++;
                  console.log(`리스팅 ${id} 평점 업데이트 완료`);
                }

                if (idx === listingIds.length - 1) {
                  updateStmt.finalize();

                  // 결과 확인
                  db.all('SELECT COUNT(*) as count FROM reviews', (err, rows) => {
                    if (!err) {
                      console.log(`총 리뷰 개수: ${rows[0].count}`);
                    }
                    resolve();
                  });
                }
