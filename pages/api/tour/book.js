...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // 필수 필드 검증
    if (!listing_id || !user_email || !user_name || !tour_date) {
    ...
    if (Math.abs(serverCalculatedTotal - (total_amount || 0)) > 1) {
      console.error('❌ [Tour Book] 금액 조작 감지:', {
      ...
      return res.status(400).json({
        ...
        message: '투어 가격이 변경되었습니다. 페이지를 새로고침해주세요.',
        expected: serverCalculatedTotal,
        received: total_amount
      });
    }
    ...
    // 4. 예약 번호 생성
    const bookingNumber = `TOUR-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
    ...
    console.log('✅ [Tour Book] 예약 생성 완료:', bookingNumber);
    ...
    console.error('❌ [Tour Book] Error:', error);
