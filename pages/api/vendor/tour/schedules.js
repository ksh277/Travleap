/**
 * 벤더 - 투어 일정 관리 API
 * GET /api/vendor/tour/schedules - 내 투어 일정 조회
 * POST /api/vendor/tour/schedules - 새 일정 생성
 * PUT /api/vendor/tour/schedules - 일정 수정
 * DELETE /api/vendor/tour/schedules - 일정 삭제
 */
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      console.error('❌ [Vendor Tour Schedules GET] Error:', error);
    ...
    try {
      ...
      if (!packageCheck.rows || packageCheck.rows.length === 0) {
        return res.status(403).json({
          success: false,
          error: '본인의 투어 패키지만 일정을 생성할 수 있습니다.'
        });
      }
      ...
      console.log(`✅ [Vendor Tour Schedule] 생성 완료: package_id=${package_id}, date=${departure_date} by vendor ${vendor_id}`);
      ...
      console.error('❌ [Vendor Tour Schedules POST] Error:', error);
      return res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  if (req.method === 'PUT') {
    try {
      ...
      const scheduleCheck = await connection.execute(`
        SELECT ts.current_participants, ts.status, tp.vendor_id
        FROM tour_schedules ts
        LEFT JOIN tour_packages tp ON ts.package_id = tp.id
        WHERE ts.id = ?
      `, [schedule_id]);
      ...
      if (currentSchedule.vendor_id !== parseInt(vendor_id)) {
        return res.status(403).json({
          success: false,
          error: '본인의 투어 일정만 수정할 수 있습니다.'
        });
      }

      // 예약이 있는데 취소하려는 경우 경고
      if (status === 'canceled' && currentSchedule.current_participants > 0) {
        console.warn(`⚠️ [Vendor Tour Schedule] 예약이 있는 일정 취소: schedule_id=${schedule_id}, bookings=${currentSchedule.current_participants}`);
      }
      ...
      console.log(`✅ [Vendor Tour Schedule] 수정 완료: schedule_id=${schedule_id} by vendor ${vendor_id}`);
      ...
      console.error('❌ [Vendor Tour Schedules PUT] Error:', error);
      return res.status(500).json({
        success: false,
        error: error.message
      });
  ...
  if (req.method === 'DELETE') {
    try {
      ...
      // 일정이 벤더 소유인지 확인
      const scheduleCheck = await connection.execute(`
        SELECT ts.current_participants, tp.vendor_id
        FROM tour_schedules ts
        LEFT JOIN tour_packages tp ON ts.package_id = tp.id
        WHERE ts.id = ?
      `, [schedule_id]);
      ...
      if (currentSchedule.vendor_id !== parseInt(vendor_id)) {
        return res.status(403).json({
          success: false,
          error: '본인의 투어 일정만 삭제할 수 있습니다.'
        });
      }

      if (currentSchedule.current_participants > 0) {
        return res.status(400).json({
          success: false,
      ...
      console.log(`✅ [Vendor Tour Schedule] 삭제 완료: schedule_id=${schedule_id} by vendor ${vendor_id}`);
      ...
      console.error('❌ [Vendor Tour Schedules DELETE] Error:', error);
      return res.status(500).json({
        success: false,
        error: error.message
      });
