...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.log('🎨 [Experience Book] 예약 요청:', {
      listing_id,
      user_id,
      experience_date,
      experience_time,
    ...
    // 필수 필드 검증
    if (!listing_id || !user_email || !user_name || !experience_date) {
    ...
    console.log('💰 [Experience Book] 가격 검증:', {
    ...
    if (Math.abs(serverCalculatedTotal - (total_amount || 0)) > 1) {
      console.error('❌ [Experience Book] 금액 조작 감지:', {
      ...
      return res.status(400).json({
        ...
        message: '체험 가격이 변경되었습니다. 페이지를 새로고침해주세요.',
        expected: serverCalculatedTotal,
        received: total_amount
      });
    }
    ...
    console.log('✅ [Experience Book] 예약 생성 완료:', bookingNumber);
    ...
    console.error('❌ [Experience Book] Error:', error);
