...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // 필수 필드 검증
    if (!listing_id || !user_email || !user_name || !reservation_date) {
    ...
    if (Math.abs(serverCalculatedTotal - (total_amount || 0)) > 1) {
      console.error('❌ [Food Book] 금액 조작 감지:', {
      ...
      return res.status(400).json({
        ...
        message: '메뉴 가격이 변경되었습니다. 페이지를 새로고침해주세요.',
        expected: serverCalculatedTotal,
        received: total_amount
      });
    }
    ...
    console.log('✅ [Food Book] 예약 생성 완료:', bookingNumber);
    ...
    console.error('❌ [Food Book] Error:', error);
